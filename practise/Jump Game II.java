...
和jumpgame1连在一起，为什么这道题可以用BFS，是因为这道题的路径！就是走下一步的时候，下一步其实是选出来的。
怎么挑的呢，如果当前层没有走完就只能先把当前层走完，在走完当前层的过程中可能会让下一步的选择集发现改变。
下一步从哪些里面选？其实应该是至今为止产生的所有的max。
这就是为什么jump1里面只用了max而没有用一种典型的BFS
但是jump2因为要记录步数。
一步的意思是什么，其实就是更新一次max。
然后下一次在更新的max这个范围内再去更新一个max。
但是计算下一个max，我们是从所有当前的里面挑出来的。
所以用bfs是非常恰当的。
所以jump1求的是下一步的集合域就好了
但是jump2的思想其实,广度优先的本质，是要把当前广度全部走完，才能得到下一个广度的范围。
求有多少步，的本质是，两个选择集没有交集。
